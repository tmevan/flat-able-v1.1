...
export class HorizontalTimelineComponent implements AfterViewInit {
  ...
  updateTimelinePosition(string, element) {
    ...
    const timelineTranslate = HorizontalTimelineComponent.getTranslateValue(this.eventsWrapper.nativeElement);
  ...
  private setDatePosition(elements: TimelineElement[], min: number, eventsMinLapse: number) {
    ...
    for (const component of elements) {
      ...
      const aWidth = HorizontalTimelineComponent.getElementWidth(timelineEventsArray[i].nativeElement);
  ...
  setTimelineWidth(elements: TimelineElement[], width, eventsMinLapse: number) {
    const timeSpan = HorizontalTimelineComponent.dayDiff(elements[0].date, elements[elements.length - 1].date);
